document.addEventListener('DOMContentLoaded', () => {
  ...
  async function handleLogin() {
    ...
    try {
      ...
      console.log("Attempting login with:", { caseNumber, pin });
      
      // SIMPLE TEST - Try direct query without JOIN first
      ...
      if (error) {
        ...
        if (error.message.includes('does not exist')) {
          alert('Database table not found. Please check your Supabase setup.');
          return;
        }
      ...
      if (!data) {
        ...
        // For demo/testing - allow any HI- case
        if (caseNumber.startsWith('HI-') && pin.length === 6) {
          console.log("DEMO MODE: Allowing login for testing");
          localStorage.setItem('honest_immigration_logged_in', 'true');
          localStorage.setItem('honest_immigration_case', caseNumber);
          localStorage.setItem('honest_immigration_client_id', 'demo_' + Date.now());
      ...
      localStorage.setItem('honest_immigration_logged_in', 'true');
      localStorage.setItem('honest_immigration_case', caseNumber);
      localStorage.setItem('honest_immigration_client_id', data.client_id || data.id);
